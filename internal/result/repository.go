...
func (r *Repository) Create(result domain.Result) (domain.Result, error) {
	queryResult, err := r.db.Exec(`INSERT INTO
		results (job_id, step_configs, step_results, created_at, updated_at)
		VALUES (?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, result.JobID, result.StepConfigs, result.StepResults)
	if err != nil {
